...
Stdlib only. Optional fields may be absent; malformed structured exports fail the ingest.
"""
    ...
    with closing(mailbox.mbox(path, create=False)) as messages:
        for key in messages.iterkeys():
            ...
            try:
                ...
                raise ValueError("Cannot parse mbox message") from exc
        ...
        for convo in _load_twitter_js(dm):
            ...
            for msg in c.get("messages", []):
                ...
                yield {
                    ...
                    "ts": _iso(datetime.datetime.fromisoformat(m["createdAt"].replace("Z", "+00:00"))) if m.get("createdAt") else None,
...
def parse_slack_export(root, workspace="", my_ids=()):
    users = {}
...
def parse_whatsapp_ios(chatstorage_path, my_name="Me"):
    ...
    con = open_readonly(chatstorage_path)
    con.row_factory = sqlite3.Row
